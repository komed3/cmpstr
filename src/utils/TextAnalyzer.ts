...
export class TextAnalyzer {
   ...
   * 
   * @returns {{ total: number, mono: number, max: number, min: number, avg: number, median: number }} - Computed syllable stats
   */
  ...
  public getMostCommonWords ( limit: number = 5 ) : string[] {
    return [ ...this.wordHistogram.entries() ].sort( ( a, b ) => b[ 1 ] - a[ 1 ] )
  ...
  public getHapaxLegomena () : string[] {
    return [ ...this.wordHistogram.entries() ].filter( ( [ , c ] ) => c === 1 )
      .map( e => e[ 0 ] );
  }
  ...
  public hasNumbers () : boolean {
    return TextAnalyzer.REGEX.number.test( this.text );
  }
